...
test('all primary UI actions have an explicit route', () => {
  ...
  for (const action of actions) {
    ...
    has(router, `case '${action}'`, `router missing ${action}`);
  }
});
...
test('market functions expose snapshot, bars, clock and streaming contracts', () => {
  for (const term of ['/api/market/snapshot', '/api/market/bars', '/api/market/clock', '/api/market/stream-config', '/api/market/stream']) {
    has(worker, term);
  }
...
test('production UI includes legal/support surfaces', () => {
  for (const file of ['public/login.html', 'public/privacy.html', 'public/terms.html', 'public/disclaimer.html', 'public/support.html']) {
